...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function leaveRoom(isCreator) {
        ...
        // Send leave message
        stompClient.send(CONFIG.SOCKET.ENDPOINTS.LEAVE, {}, JSON.stringify({
            type: 'LEAVE_ROOM',
            roomCode: currentRoomCode,
            senderId: firebaseUid,
            senderUsername: playerUsername
    ...
    function handleRoomJoined(message) {
        logMessage(`${message.senderUsername} joined the room`, 'info');
        ...
        if (isHost) {
            ...
            // Update joined room info
            elements.join.joinedRoomCode.textContent = currentRoomCode;
    ...
    function handlePlayerLeft(message) {
        ...
        // If we're in a game, return to lobby
